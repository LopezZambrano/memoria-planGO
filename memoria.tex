...
\item Gen\'ericos: muy útiles para hacer c\'odigo mas reusable, especialmente en listas y Arrays.
...
Tiene la capacidad de concatenar varios ficheros en uno. Tambi\'on ofrece la posibilidad de generar sourcemaps lo cual nos mapea el c\'odigo compilado a ficheros sin compilar para que haga el proceso de debuguear mas sencillo. Por útimo añadir que genera un c\'odigo modular.
...
Como se ha explicado m\'as detalladamente en \ref{subsec:estilo}, TypeScript es m\'as interesante que el uso de JavaScript ya que permite tipado est\'atico, hacer gen\'ericos y decorators.
...
Para inyectar, se hace a través del contructor.
...
Angular crea por cada componente un detector de cambios en tiempo de ejecición adaptado a la estructura concreta de cada uno de ellos. Esto permite una gran optimización de dichos detectores de cambios.

Los cambios de propagan a los componentes hij
...
En el cap\'itulo 3 se explica el dise\'no e implementacion del proyecto. De la parte frontend se detalla el mapa 
de navegac\'ion que contiene la aplicaci\'on, el dise\'no de cada pantalla con su correspondiente funcionalidad y la 
...
\begin{itemize}
\item Primer mes: mi dedicaci\'on durantes este tiempo fue a la realizaci\'on de un tutorial sobre Angular, el cual me di\'o una serie de conocimientos para poder empezar la parte visual de la aplicaci\'on. Tras este tutorial, empec\'e a crear las primeras pantallas de la aplicaci\'on.

 \item Segundo mes: este mes fue dedicado a la realizaci\'on de un tutorial destinado a conectar Angular con Meteor para poder desarrollar la parte backend con Meteor. Durante este periodo, debido a una serie de problemas con la realizaci\'on de este tutorial, decid\'i buscar otra alternativa y as\'i poder avanzar en el proyecto. Finalmente la tecnolog\'ia elegida fue Express. Esto me permiti\'o poder realizar las primeras llamadas HTTP desde la parte frontend de la aplicaci\'on.

\item Tercer y cuarto mes: una vez montada la base de la parte front y la parte back, desarrolle en paralelo ambas partes. Desarrollaba la pantalla y a la par,  creaba los controladores necesarios en Express para que atendiera a las peticiones necesarias en dicha pantalla. Una vez terminado todo el desarrollo, fue importante la parte del testeo de la aplicaci\'on. Durante esta parte se cambiaron funcionalidades de la aplicaci\'on como: poder a\'nadir hora como opci\'on en una encuesta, ordenar alfab\'eticamente los listados, cambio en pantalla de visualizo de votos, etc.

\item Quinto mes: este ultimo tramo ha sido dedicado a la elaboraci\'on de la memoria y la preparaci\'on de la presentaci\'on. Adem\'as de esto, durante esta etapa, se hizo el experimento de poner a un grupo de personas a usar la aplicaci\'on siguiendo una serie de hitos, para posteriormente contestar unas preguntas.
...
Como vemos en la 4.2a y 4.2b el menu lateral y el icono flecha no se muestran a la vez.
...
tecnolog\'ia Ionic. Como ya hemos dicho, la parte en la que se lleva a cabo todo el desarrollo es en la carpeta 'src'. Por esto vamos a ver mas detalladamente
que se encuentra dentro de esta carpeta.

\begin{figure}
  \centering
  \includegraphics[width=4cm, keepaspectratio]{img/arq_src.png}
  \caption{Arquitectura carpeta 'src'}
  \label{figura:arq_src}
\end{figure}
...
En esta carpeta encontramos una carpeta variables y los ficheros que inicializan la aplicaci\'on. 

En la carpeta de variables hay una serie de ficheros que contienen las  variables comunes utilizadas en toda la aplicaci\'on para el dise\'no. De esta manera los colores son iguales,
 tama\'nos e iconos son comunes en toda la app. Este es un ejemplo de buenas pr\'acticas, si fuera necesario cambiar la apariencia de toda la aplicaci\'on, bastaria con modificar alguna de estas variables.

Con respecto a los ficheros, uno de gran importancia para la aplicaci\'on es el app.module.ts, ya que en \'el est\'an declarados todos los componentes y servicios que han sido creados. Para reglas de CSS que sean comunes para toda la aplicaci\'on, se utiliza el fichero app.css. 
Por \'utimo, otro fichero a comentar es el app.component.ts en el que se encuentra la inicializaci\'on de la aplicaci\'on.


\begin{figure}
  \centering
  \includegraphics[width=4cm, keepaspectratio]{img/arq_app.png}
  \caption{Arquitectura carpeta 'app'}
  \label{figura:arq_app}
\end{figure}

\subsubsection{assets} 
\label{sec:assets}

Aqu\'i se encuentran todas las imagenes utilizadas en la aplicaci\'on.
